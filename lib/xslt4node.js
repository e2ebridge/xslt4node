...
function newFileInputStreamSync(path) {
    return java.newInstanceSync('java.io.FileInputStream', path);
}
...
module.exports.transformSync = function (config) {
    ...
    if (config.xsltPath !== undefined || config.xslt !== undefined) {
        if (config.xsltPath) {
            xsltSource = newFileInputStreamSync(config.xsltPath);
        } else if (typeof config.xslt === 'string') {
    ...
    if (config.sourcePath) {
        source = newFileInputStreamSync(config.sourcePath);
    } else if (typeof config.source === 'string') {
    ...
    transformer.transformSync(newStreamSourceSync(source), newStreamResultSync(result));
    source.closeSync();
    result.closeSync();
